...
- Sales pipeline: `/admin/sales` starts with the sales agents page for client sale capture (surname, ID, address, Persal, department, first debit date, dependants), submit-sale validation checks, validation-agent edit/correction, and final validation handoff into T status/QA bay language. It then shows FoxPro-inspired stages (`Sales Capture`, `In QA Validation`, `QA Passed`, `Exported Awaiting Outcome`, `Q-Link Uploaded`, `Repair`, `Client Cancelled`) and preserves raw FoxPro status labels from synced data.
